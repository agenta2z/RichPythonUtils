...
def test_regex_with_compilation():
    ...
    assert string_compare("abc", pattern, option) == False
    print(
        f"✓ Compiled regex exact match works (pattern type: {type(pattern).__name__})"
    )
...
def test_direct_pattern_passing():
    ...
    from rich_python_utils.string_utils.comparison import CompareMethod, CompareOption

    # Create a pre-compiled pattern
    compiled = re.compile(r"\d+")
    option = CompareOption(
        is_regular_expression=True, compare_method=CompareMethod.ExactMatch
    )
