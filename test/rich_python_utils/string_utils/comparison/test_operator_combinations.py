...
def test_operator_combination(pattern_str, test_cases, compile_regex=True):
    ...
    for test_str, expected in test_cases:
        ...
        print(
            f"    {status} string_compare('{test_str}', pattern, option) = {result} (expected {expected})"
        )
    ...
    test_operator_combination(
        ...
        [
            (
                "ABC",
                True,
            ),  # Case sensitive, doesn't start with lowercase, negation = True
