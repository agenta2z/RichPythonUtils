...
class TestMappingToXmlRealData:
    ...
    def test_action_results_conversion(self):
        ...
        assert "<ActionResults>" in result_xml, (
            "Should contain opening root tag <ActionResults>"
        )
        assert "</ActionResults>" in result_xml, (
            "Should contain closing root tag </ActionResults>"
        )
        ...
        assert "<ActionResult>" in result_xml, (
            "Should contain opening tag <ActionResult>"
        )
        assert "</ActionResult>" in result_xml, (
            "Should contain closing tag </ActionResult>"
        )

        # Validate some expected content from the markdown data
        assert "Slack Channel Summary" in result_xml, (
            "Should contain expected content from the data"
        )
        assert "seo-link-building-news-updates" in result_xml, (
            "Should contain the Slack channel name"
        )
        ...
        assert not result_xml.startswith("<?xml"), (
            "Should not have XML declaration by default"
        )

        # Verify it starts with the root tag (possibly with whitespace)
        assert result_xml.strip().startswith("<ActionResults>"), (
            "Should start with root tag"
        )
        assert result_xml.strip().endswith("</ActionResults>"), (
            "Should end with root tag"
        )
    ...
    def test_invalid_input_type_raises_value_error(self):
        ...
        assert (
            "Expected a Mapping, Sequence of Mappings, or an attrs class instance"
            in error_message
        )
