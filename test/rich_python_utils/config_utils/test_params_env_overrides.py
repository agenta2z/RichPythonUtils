...
class TestPrefix:
    ...
    def test_derived_from_configs_folder(self):
        assert (
            _resolve_env_prefix({"_params": {}}, "/root/task/configs/x.yaml") == "TASK"
        )
    ...
    def test_external_override_wins_over_config_and_folder(self):
        ...
        assert (
            _resolve_env_prefix(c, "/root/task/configs/x.yaml", "RESEARCH_PROPOSE")
            == "RESEARCH_PROPOSE"
        )
    ...
    def test_external_override_uppercased(self):
        assert (
            _resolve_env_prefix({"_params": {}}, "/x/x.yaml", "research_propose")
            == "RESEARCH_PROPOSE"
        )
...
def _out(f, overrides=None):
    return OmegaConf.to_container(
        load_config(str(f), overrides=overrides), resolve=True
    )["out"]
...
def test_scalar_override(cfg_file, clean_env):
    ...
    assert out["flows"] == [
        "DevmateCLI",
        "DevmateCLI",
    ]  # propagates via ${main_inferencer}
...
def test_config_defaults_are_below_env_and_cli(cfg_file, clean_env):
    ...
    out = OmegaConf.to_container(
        load_config(str(cfg_file), config_defaults=cd), resolve=True
    )["out"]
    ...
    out = OmegaConf.to_container(
        load_config(
            str(cfg_file),
            overrides={"_params.plan_max_breakdown": 99},
            config_defaults=cd,
        ),
        resolve=True,
    )["out"]
...
def test_config_defaults_none_is_noop(cfg_file, clean_env):
    ...
    b = OmegaConf.to_container(
        load_config(str(cfg_file), config_defaults=None), resolve=True
    )
    assert a == b
