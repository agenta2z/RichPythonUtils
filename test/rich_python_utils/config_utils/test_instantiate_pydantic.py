...
from test_helpers import ParentAttrs, SimplePydantic, ValidatedPydantic
...
class TestPydantic:
    def test_from_target(self):
        cfg = OmegaConf.create(
            {"_target_": f"{_MOD}.SimplePydantic", "name": "hi", "value": 5}
        )
        obj = instantiate(cfg)
    ...
    def test_with_validators(self):
        ...
        cfg_bad = OmegaConf.create(
            {"_target_": f"{_MOD}.ValidatedPydantic", "score": -1}
        )
        ...
        cfg = OmegaConf.create(
            {
                ...
                "child": {
                    "_target_": f"{_MOD}.SimplePydantic",
                    "name": "pyd",
                    "value": 7,
                },
                "label": "mixed",
