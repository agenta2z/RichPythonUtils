...
class TestUpdateValuesBasic:
    ...
    def test_update_numbers_in_nested_structure(self):
        ...
        result = update_values(
            lambda x: x * 2 if isinstance(x, (int, float)) else x, data
        )
    ...
    def test_update_nested_dictionaries(self):
        ...
        result = update_values(lambda x: x + 10 if isinstance(x, int) else x, data)
        assert result == {"a": 11, "b": {"c": 12, "d": [13, 14]}}

    def test_handle_tuples(self):
        """Test handling tuples (converted to lists)"""
        result = update_values(
            lambda x: x * 2 if isinstance(x, int) else x, (1, 2, (3, 4))
        )
    ...
    def test_handle_none_values(self):
        ...
        result = update_values(
            lambda x: "null" if x is None else x, [1, None, [2, None]]
        )
    ...
    def test_complex_nested_structure(self):
        ...
        expected = {
            "users": [{"name": "ALICE", "age": 60}, {"name": "BOB", "age": 50}],
            "count": 4,
        }
...
class TestUpdateValuesInPlace:
    ...
    def test_inplace_update_dict(self):
        ...
        result = update_values(
            lambda x: x * 2 if isinstance(x, int) else x, data, inplace=True
        )
    ...
    def test_non_inplace_update(self):
        ...
        result = update_values(
            lambda x: x * 2 if isinstance(x, int) else x, data, inplace=False
        )
...
class TestUpdateValuesAtomTypes:
    ...
    def test_custom_atom_types_tuple(self):
        ...
        result = update_values(
            lambda x: x * 2 if isinstance(x, int) else x, data, atom_types=(str, tuple)
        )
...
class TestUpdateValuesEdgeCases:
    ...
    def test_float_values(self):
        ...
        result = update_values(
            lambda x: round(x * 1.1, 2) if isinstance(x, float) else x, data
        )
