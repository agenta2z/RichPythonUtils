...
def test_concurrent_producers():
    ...
    try:
        ...
        for i in range(num_producers):
            p = mp.Process(
                target=concurrent_producer, args=(tmpdir, i, items_per_producer, 0.01)
            )
            p.start()
            processes.append(p)
        ...
        assert final_size == expected_size, (
            f"Expected {expected_size} items, got {final_size}"
        )
        ...
        for producer_id in range(num_producers):
            ...
            assert producer_items == expected_items, (
                f"Producer {producer_id} has missing/duplicate items"
            )
...
def test_concurrent_consumers():
    ...
    try:
        ...
        for i in range(num_consumers):
            p = mp.Process(
                target=concurrent_consumer, args=(tmpdir, i, 2.0, results_queue)
            )
            p.start()
            processes.append(p)
        ...
        assert len(all_items) == num_items, (
            f"Expected {num_items}, got {len(all_items)}"
        )
...
def test_producer_consumer_concurrent():
    ...
    try:
        ...
        for i in range(num_producers):
            p = mp.Process(
                target=concurrent_producer, args=(tmpdir, i, items_per_producer, 0.02)
            )
            p.start()
            processes.append(p)
        ...
        for i in range(num_consumers):
            p = mp.Process(
                target=concurrent_consumer, args=(tmpdir, i, 5.0, results_queue)
            )
            p.start()
            processes.append(p)
        ...
        assert len(all_consumed_items) + remaining == total_items, (
            "Items lost or duplicated"
        )

        # Check for duplicates
        duplicates = [
            item for item, count in Counter(all_consumed_items).items() if count > 1
        ]
...
def test_lock_stress():
    ...
    try:
        ...
        for i in range(num_workers):
            p = mp.Process(
                target=lock_stress_worker,
                args=(tmpdir, i, ops_per_worker, results_queue),
            )
            p.start()
            processes.append(p)
        ...
        for _ in range(num_workers):
            ...
            print(
                f"   [OK] Worker {worker_id}: {ops_completed} ops, {error_count} errors"
            )
        ...
        assert success_rate >= 90.0, (
            f"Success rate {success_rate:.1f}% too low (need >90%)"
        )
...
def test_fifo_ordering_under_concurrency():
    ...
    try:
        ...
        print(
            f"   [OK] Created {num_items} items in order: 0, 1, 2, ..., {num_items - 1}"
        )
        service.close()
