...
import pytest
from rich_python_utils.io_utils.json_io import JsonLogger, PartsKeyPath, write_json
...
def _make_session_logger(
    ...
    turn_aware_kwargs=None,
    cross_turn=False,
    cross_turn_kwargs=None,
    **kwargs,
):
    ...
    turn_logger = JsonLogger(
        file_path=turn_log_file, append=True, **(turn_aware_kwargs or {})
    )
    sl.add_turn_aware_logger(turn_logger)

    if cross_turn:
        cross_log_file = str(sl.session_dir / "cross.jsonl")
        cross_logger = JsonLogger(
            file_path=cross_log_file, append=True, **(cross_turn_kwargs or {})
        )
