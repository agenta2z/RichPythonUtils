...
def test_workgraph_node_serialization_preserves_config(node: WorkGraphNode):
    ...
    assert serializable_obj["name"] == node.name, (
        f"Name mismatch: {serializable_obj['name']} != {node.name}"
    )
    ...
    assert config["min_repeat_wait"] == node.min_repeat_wait, (
        f"min_repeat_wait mismatch"
    )
    assert config["max_repeat_wait"] == node.max_repeat_wait, (
        f"max_repeat_wait mismatch"
    )
...
def test_workgraph_node_value_reference_handling(node: WorkGraphNode):
    ...
    if value_ref is not None:
        assert isinstance(value_ref, str), (
            f"value_ref should be string or None, got {type(value_ref)}"
        )
        assert "." in value_ref, (
            f"value_ref should be in 'module.name' format, got {value_ref}"
        )
