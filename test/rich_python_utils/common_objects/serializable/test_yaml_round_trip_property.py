...
tags_strategy = st.lists(safe_text_strategy, min_size=0, max_size=5)
...
def test_yaml_round_trip_preserves_simple_data(obj: YamlSimpleSerializable):
    ...
    assert restored.value == obj.value, (
        f"Value mismatch: {restored.value} != {obj.value}"
    )
    assert restored.enabled == obj.enabled, (
        f"Enabled mismatch: {restored.enabled} != {obj.enabled}"
    )
...
def test_yaml_round_trip_preserves_nested_data(obj: YamlNestedSerializable):
    ...
    assert restored.title == obj.title, (
        f"Title mismatch: {restored.title} != {obj.title}"
    )
    assert restored.count == obj.count, (
        f"Count mismatch: {restored.count} != {obj.count}"
    )
    assert restored.items == obj.items, (
        f"Items mismatch: {restored.items} != {obj.items}"
    )
    assert restored.config == obj.config, (
        f"Config mismatch: {restored.config} != {obj.config}"
    )
...
def test_yaml_round_trip_preserves_complex_data(obj: YamlComplexSerializable):
    ...
    assert abs(restored.score - obj.score) < 1e-6, (
        f"Score mismatch: {restored.score} != {obj.score}"
    )
    assert restored.tags == obj.tags, f"Tags mismatch: {restored.tags} != {obj.tags}"
    assert restored.metadata == obj.metadata, (
        f"Metadata mismatch: {restored.metadata} != {obj.metadata}"
    )
    assert restored.description == obj.description, (
        f"Description mismatch: {restored.description} != {obj.description}"
    )
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        ("YAML round-trip multiple cycles", test_yaml_round_trip_multiple_cycles),
    ]
