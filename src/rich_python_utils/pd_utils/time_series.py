...
def time_series_stats(
    ...
    if cols_to_add_as_pct_change:
        ...
        df_pct_change.columns = [
            col + added_pct_change_col_suffix for col in df_pct_change.columns
        ]
        df = pd.concat([df, df_pct_change], axis=1)

    if cols_to_overwrite_as_pct_change:
        if pct_change_multiply_100:
            df[list(cols_to_overwrite_as_pct_change)] = (
                df[list(cols_to_overwrite_as_pct_change)].pct_change() * 100
            )
        else:
            df[list(cols_to_overwrite_as_pct_change)] = df[
                list(cols_to_overwrite_as_pct_change)
            ].pct_change()
    ...
    for window in windows:
        df_stats = (
            df[stat_cols]
            .rolling(window)
            .agg({stat_col: stats for stat_col in stat_cols})
        )
