...
from rich_python_utils.datetime_utils.common import (
    solve_date_time_format_by_granularity,
)
...
def _solve_paras_from_io_mode(
    file: str, mode: str, use_tqdm: bool, description: Optional[str], verbose: bool
) -> Tuple[bool, str, bool]:
    """
...
def read_text_or_file(
    text_or_file: str,
    read_text_func: Callable[[str], Any],
    read_file_func: Callable[[str], Any],
):
...
def backup(
    ...
    if not os.path.exists(input_path):
        raise FileNotFoundError(
            f"The input file or directory {input_path} does not exist."
        )
    ...
    solved_date_format, solved_time_format = solve_date_time_format_by_granularity(
        datetime_granularity, date_format, time_format
    )
