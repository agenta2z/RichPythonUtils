...
class OnStorageLists:
    ...
    def _archive_item(self, item_path: Path) -> None:
        ...
        archived_name = (
            f"{timestamp_str}_{original_name}_{random_suffix}{item_path.suffix}"
        )
        ...
        if index is not None:
            ...
            if item_path.exists() and not overwrite:
                raise FileExistsError(
                    f"Item at index {index} already exists. Use overwrite=True to replace."
                )
            ...
            if not isinstance(obj_or_list, (list, tuple)):
                raise TypeError(
                    "When index is None, obj_or_list must be a list or tuple"
                )
            if overwrite:
                self.clear(list_key)
    ...
    def pop(
        self, index: int = -1, list_key: Optional[str] = None, default: Any = None
    ) -> Any:
