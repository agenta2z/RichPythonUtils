...
def get_pairwise_metrics(
    ...
    if metric_names:
        ...
        for metric_name in metric_names:
            ...
            if (
                unpack_single
                and not isinstance(metric_val, float)
                and (metric_val.shape == (1, 1) or metric_val.shape == (1,))
            ):
                metric_val = (
                    round(float(metric_val), decimals)
                    if decimals is not None
                    else float(metric_val)
                )
            elif flatten:
                metric_val = (
                    [round(float(x), decimals) for x in metric_val.flatten()]
                    if decimals is not None
                    else [float(x) for x in metric_val.flatten()]
                )
