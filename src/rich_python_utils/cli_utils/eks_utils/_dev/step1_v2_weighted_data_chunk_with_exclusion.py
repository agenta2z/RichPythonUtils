...
output_path = f'{path.dirname(input_path_root)}/source_data_chunked/chunk_{chunk_size}'
...
input_paths = list(
    filter(
        lambda input_path: not any(excluded_input_path in input_path for excluded_input_path in excluded_input_path_keys),
        input_paths_all
    )
)

hprint_message(
    'len_input_paths_all', len(input_paths_all),
    'included_input_path_keys', len(excluded_input_path_keys),
    'len_input_paths', len(input_paths)
)

output_path = output_path + '_exclude_' + hashlib.sha1(' '.join(input_paths).encode()).hexdigest()
