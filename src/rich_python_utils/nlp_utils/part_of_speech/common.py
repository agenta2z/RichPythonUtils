...
def get_flair_pos_tagger_name_by_language(
    language: Union[str, Languages], fast: bool = True
):
    if fast:
...
def get_pos_tagger(
    tool: PreDefinedNlpToolNames, language: Languages = Languages.English, **kwargs
):
    if tool == PreDefinedNlpToolNames.NLTK:
        ...
        return partial(
            _pos_tag, tagger=_get_tagger(language_pos_tagger), lang=language_pos_tagger
        )
    elif tool == PreDefinedNlpToolNames.FLAIR:
        from flair.models import SequenceTagger

        pos_tagger_name = get_flair_pos_tagger_name_by_language(
            language=language, fast=kwargs.get("fast", True)
        )
...
def get_pos_tagging_method(tool: PreDefinedNlpToolNames, **kwargs):
    ...
    hprint_message(
        ("pos_tag_method", pos_tag_), *_pos_tag_args.items(), title="pos_tag_args"
    )
