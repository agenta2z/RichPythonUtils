...
from rich_python_utils.string_utils.regex import (
    get_regex_match_group_indexes,
    iter_matches,
)

_NUMBERS_REGEX_EN = (
    "(?P<n100>100th|100|a hundred|one hundred|hundred|hundreds|hundredth)|"
...
_NUMBERS_REGEX_ES = (
    r"(?P<n11>(?:(?:la|los|de)\s)?(?:(?:11|once)|(?:(?:undécim|undecim)(?:a|o)?)))|"
...
_NUMBERS_COMPLIED_REGEX_EN = re.compile(rf"\b({_NUMBERS_REGEX_EN})\b")
_NUMBERS_COMPLIED_REGEX_DE = re.compile(rf"\b({_NUMBERS_REGEX_DE})\b")
_NUMBERS_COMPLIED_REGEX_ES = re.compile(rf"\b({_NUMBERS_REGEX_ES})\b")
...
def has_conflict_numbers(
    ...
    **kwargs,
    ...
    return compare_sets(
        utt1_nums, utt2_nums, allows_add=allows_add, allows_drop=allows_drop
    ) and (
        edit_distance_threshold_to_ignore_conflict is None
        or edit_distance(
            *remove_common_tokens_except_for_sub_tokens(
