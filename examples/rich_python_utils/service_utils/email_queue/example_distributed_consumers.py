...
def producer(service, queue_id, num_items=10):
    ...
    for i in range(num_items):
        item = {"task_id": i, "task": f"process_item_{i}", "timestamp": time.time()}
...
def main():
    ...
    # Create email client
    email_client = GmailClient(credentials_path=credentials_path, token_path=token_path)

    # Create storage service (unique per process)
    storage_service = StorageBasedQueueService(
        root_path=storage_path, archive_popped_items=True
    )
