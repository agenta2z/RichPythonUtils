...
def main():
    ...
    try:
        ...
        # 6. Namespaces
        svc.add(
            Document(doc_id="ns_doc", content="Physics experiment data"),
            namespace="physics",
        )
        svc.add(
            Document(doc_id="ns_doc", content="Biology experiment data"),
            namespace="biology",
        )
    ...
    print(
        f"    'renewable energy and battery technology' -> {len(results_energy_formatted)} results:"
    )
