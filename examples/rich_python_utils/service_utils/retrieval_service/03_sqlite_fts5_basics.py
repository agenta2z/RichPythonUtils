...
def main():
    ...
    try:
        ...
        results_gene_editing_formatted = [
            (d.doc_id, s) for d, s in results_gene_editing
        ]

        results_climate_model = svc.search('"climate model"')
        results_climate_model_formatted = [
            (d.doc_id, s) for d, s in results_climate_model
        ]
        ...
        with SQLiteFTS5RetrievalService(db_path=":memory:") as mem_svc:
            mem_svc.add(
                Document(doc_id="tmp", content="temporary FTS5 document for testing")
            )
        ...
        print(
            f"    'protein structure prediction' -> {len(results_protein_formatted)} results:"
        )
        ...
        print("-" * 50)
        print(
            f"    '\"gene editing\"' -> {len(results_gene_editing_formatted)} results:"
        )
        for rid, score in results_gene_editing_formatted:
            print(f"        {rid:35s} score={score:.4f}")
        print(
            f"    '\"climate model\"' -> {len(results_climate_model_formatted)} results:"
        )
