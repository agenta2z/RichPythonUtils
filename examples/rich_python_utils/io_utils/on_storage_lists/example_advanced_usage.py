...
def example_hierarchical_organization():
    ...
    try:
        storage = OnStorageLists(
            root_path=tmpdir, default_list_key="default", list_key_components_sep="."
        )
        ...
        storage.append(
            {"theme": "dark", "notifications": True}, list_key="users.alice.settings"
        )
        ...
        storage.append(
            {"theme": "light", "notifications": False}, list_key="users.bob.settings"
        )

        # Application data
        storage.append(
            {"version": "1.0", "debug": False}, list_key="app.config.production"
        )
        storage.append(
            {"version": "1.0-dev", "debug": True}, list_key="app.config.development"
        )
...
def example_complex_data_structures():
    ...
    try:
        storage = OnStorageLists(root_path=tmpdir, default_list_key="complex_data")
        ...
        order2 = {
            ...
            "items": [{"product": "Doohickey", "quantity": 3, "price": 9.99}],
            "total": 29.97,
            "status": "pending",
        }
...
def example_performance_optimization():
    ...
    try:
        storage = OnStorageLists(root_path=tmpdir, default_list_key="data")
