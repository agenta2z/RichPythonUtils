...
def use_case_task_queue():
    ...
    try:
        storage = OnStorageLists(root_path=tmpdir, default_list_key="tasks.pending")
        ...
        completed_storage = OnStorageLists(
            root_path=tmpdir, default_list_key="tasks.completed"
        )
        ...
        print(
            f"   Completed: {completed_storage._get_list_size(list_key='tasks.completed')}"
        )
...
def use_case_config_history():
    ...
    try:
        storage = OnStorageLists(root_path=tmpdir, default_list_key="config.history")
        ...
        for i, config in enumerate(history):
            print(
                f"   [{i}] v{config['version']} - {config['timestamp']} by {config['author']}"
            )
        ...
        print(
            f"   [OK] Rolled back to v{previous['version']} as v{rollback_config['version']}"
        )
...
def use_case_user_data_management():
    ...
    try:
        ...
        storage.set(
            prefs, list_key=f"users.{user}.preferences", index=0, overwrite=True
        )
...
def use_case_event_sourcing():
    ...
    try:
        storage = OnStorageLists(root_path=tmpdir, default_list_key="events")
        ...
        state = {"account_id": None, "name": None, "balance": 0, "created_at": None}
...
def use_case_data_pipeline_checkpointing():
    ...
    try:
        storage = OnStorageLists(
            root_path=tmpdir, default_list_key="pipeline.checkpoints"
        )
        ...
        for batch in data_batches:
            ...
            print(
                f"   [✓] Batch {batch['batch_id']} completed ({batch['records']} records)"
            )
        ...
        for cp in checkpoints:
            print(
                f"   - Batch {cp['batch_id']}: {cp['records_processed']} records at {cp['timestamp']}"
            )
        ...
        print(
            f"   [OK] Pipeline can resume from batch {last_checkpoint['batch_id'] + 1}"
        )
