...
def example_basic_usage():
    ...
    result = update_values(lambda x: x * 2 if isinstance(x, (int, float)) else x, data)
    ...
    result = update_values(lambda x: 0 if x is None else x, data)
...
def example_inplace_vs_copy():
    ...
    result = update_values(
        lambda x: x * 10 if isinstance(x, int) else x, data, inplace=True
    )
    ...
    print(f"   Original ID: {id(data)}")
    result = update_values(
        lambda x: x * 10 if isinstance(x, int) else x, data, inplace=False
    )
    print(f"   Result:   {result}")
    print(f"   Result ID: {id(result)}")
    print(f"   Same object? {result is data}")
...
def example_real_world_use_cases():
    ...
    api_response = {
        ...
        "settings": {"token": "abc123", "public": True},
    }

    def sanitize_sensitive(x):
        if isinstance(x, str) and any(
            key in x.lower() for key in ["password", "secret", "key", "token"]
        ):
            return "***REDACTED***"
        return x
